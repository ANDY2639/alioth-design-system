...
export const Body = ({ type = "body1", variant, children, className = "", as = "p" }: BodyProps) => {
  const baseClasses = bodyStyles[type];
  const weightClass = variant ? variantWeights[variant] : bodyWeights[type];

  const classes = [baseClasses, weightClass, "text-neutral-900 dark:text-neutral-100", className].filter(Boolean).join(" ");
